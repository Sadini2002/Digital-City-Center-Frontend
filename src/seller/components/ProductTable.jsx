...
return (
<div
  className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white py-12 text-center">
      ...
      <tr key={id} className="hover:bg-slate-50/70 transition">
        <td className="px-5 py-3.5">
          <img src={imageUrl} alt={product.name}
            className="h-12 w-12 rounded-lg border border-slate-200 bg-slate-50 object-cover shadow-sm" onError={(e)=> {
          e.target.src =
          "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=150&auto=format&fit=crop&q=60";
          }}
          />
        ...
        <td className="px-5 py-3.5">
          ...
          <div className="mt-1 flex flex-wrap gap-1 items-center">
            ...
            <span key={color}
              className="rounded bg-slate-100 px-1 py-0.5 text-[10px] font-semibold text-slate-600 flex items-center gap-1">
        ...
        <td className="px-5 py-3.5">
          ...
          return (
          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ring-1 capitalize
            ${badgeStyles}`}>
            {label}
          </span>
          );
        ...
        <td className="px-5 py-3.5 text-right">
          <div className="inline-flex items-center gap-3">
            <Link to={`/seller/listings/${id}/edit`} state={product}
            ...
            <button type="button" onClick={()=> onDelete(id)}
              className="inline-flex items-center gap-1 rounded-lg border border-red-100 bg-red-50 px-2.5 py-1.5 text-xs
              font-semibold text-red-700 transition hover:bg-red-100/80"
              >
